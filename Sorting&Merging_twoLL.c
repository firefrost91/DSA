...
struct node* sort_and_merge_LL ( struct node*first , struct node*first2){
    ...
    while(p!=NULL && q!=NULL){
        if(p->data>q->data){
            final=add_at_last(final ,q->data);
            q=q->next ;  }
        else{
            final=add_at_last(final , p->data);
            p=p->next ; 
        }
    }
     while(p!=NULL){
...
int main(){
    ...
    first2=add_at_last(first2 , 1);
    first2=add_at_last(first2 , 13);
    display(first2); 
    struct node* final ;
    final = sort_and_merge_LL(first , first2 ); 
    display(final); 
